...
class RelocateTouch(leap_hand_base.LeapHandEnv):
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    # State size = hand joints + touch sensors + previous actions + target position (x, y only) + DEBUG cube state
    state_size = len(self._hand_qids) + self._touch_size + self.mjx_model.nu + 2 + 13  # +13 for obj_pos(3) + obj_quat(4) + obj_linvel(3) + obj_angvel(3)
  ...
  def _check_hand_object_contact(self, obj_pos: jax.Array, fingertip_positions: jax.Array) -> jax.Array:
    ...
    contact_threshold = 0.06  # the cube is 0.035 m wide, 0.05 is around 0.035 * √2, so 0.06 is a good threshold
